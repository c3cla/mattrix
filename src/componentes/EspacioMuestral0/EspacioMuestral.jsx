...
const EspacioMuestral = () => {
  ...
  useEffect(() => {
    ...
    if (etapa === 1) cantidadSimbolos = 5;
    else if (etapa === 2) cantidadSimbolos = 7;
    else if (etapa === 3) cantidadSimbolos = 9;
  ...
  const verificarRespuesta = () => {
    ...
    if (esCorrecto) {
      ...
      // Se elimina la llamada a nuevaBaraja()
    }
  };
  ...
  const cerrarRetroalimentacion = () => {
    ...
    setEspacioMuestral([]);
    setMostrarPregunta(true); // Muestra inmediatamente la etapa actual sin cuenta regresiva
    setTiempoInicio(Date.now()); // Reinicia el tiempo de inicio para medir el tiempo de respuesta correctamente
  };
    ...
    <div className="espacio-muestral-container">
      {mostrarResumen ? (
        ...
        <>
          ...
          {/* Condicional para renderizar la columna de símbolos solo cuando mostrarPregunta es true */}
          {mostrarPregunta && (
          ...
          {mostrarRetroalimentacion && (
            <div className="retroalimentacion">
              ...
              {/* Se elimina la sección de símbolos correctos */}
